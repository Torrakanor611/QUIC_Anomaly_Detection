...
async def perform_http_request(
    ...
    logger.info(
        "Response received for %s %s : %d bytes in %.1f ms (%.3f Mbps)"
        % (method, urlparse(url).path, octets, elapsed*1000, octets * 8 / elapsed / 1000000)
    )
...
async def main(
    ...
    async with connect(
        ...
        # perform multiple consecutive dummy requests
        while(True):
            coros = [
